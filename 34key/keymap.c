...
/*implementing timeout for oneshot tap lock mods*/
#if (defined(ONESHOT_LOCK_TIMEOUT) && (ONESHOT_LOCK_TIMEOUT > 0))
static uint16_t oneshot_locked_mods_timer;
static void     check_oneshot_locked_mods_timeout(void);
bool            locked_mods_timer_actived = false;

void check_oneshot_locked_mods_timeout(void) {
    if (get_oneshot_locked_mods() != 0) { // if locked mods are active
        if (locked_mods_timer_actived) {
            if (timer_elapsed(oneshot_locked_mods_timer) > ONESHOT_LOCK_TIMEOUT) clear_oneshot_locked_mods();
        } else {
            oneshot_locked_mods_timer = timer_read();
            locked_mods_timer_actived = true;
        };
    } else
        locked_mods_timer_actived = false;
}
#endif

void housekeeping_task_user(void) {
#if (defined(ONESHOT_LOCK_TIMEOUT) && (ONESHOT_LOCK_TIMEOUT > 0))
    check_oneshot_locked_mods_timeout();
#endif
};
